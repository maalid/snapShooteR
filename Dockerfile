...
RUN apt-get update && apt-get install -y  git-core libcurl4-openssl-dev libgit2-dev libssh2-1-dev libssl-dev libxml2-dev make pandoc pandoc-citeproc python2-minimal zlib1g-dev && rm -rf /var/lib/apt/lists/*
...
RUN Rscript -e 'remotes::install_version("reticulate",upgrade="never", version = "1.13")'
...
EXPOSE 3838
CMD R -e "options('shiny.port'=3838,shiny.host='0.0.0.0');snapShooteR::run_app()"
